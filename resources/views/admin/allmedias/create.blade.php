...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.allmedia.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.allmedias.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('etiquettes') ? 'is-invalid' : '' }}" type="text" name="etiquettes" id="etiquettes" value="{{ old('etiquettes', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('typeofmedia') ? 'is-invalid' : '' }}" name="typeofmedia_id" id="typeofmedia_id" required>
                    @foreach($typeofmedia as $id => $entry)
                        <option value="{{ $id }}" {{ old('typeofmedia_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
