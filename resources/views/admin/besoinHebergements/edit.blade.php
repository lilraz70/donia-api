...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.besoinHebergement.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.besoin-hebergements.update", [$besoinHebergement->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nb_chambre">{{ trans('cruds.besoinHebergement.fields.nb_chambre') }}</label>
                <input class="form-control {{ $errors->has('nb_chambre') ? 'is-invalid' : '' }}" type="number" name="nb_chambre" id="nb_chambre" value="{{ old('nb_chambre', $besoinHebergement->nb_chambre) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('prix_journalier') ? 'is-invalid' : '' }}" type="number" name="prix_journalier" id="prix_journalier" value="{{ old('prix_journalier', $besoinHebergement->prix_journalier) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('prix_mensuel') ? 'is-invalid' : '' }}" type="number" name="prix_mensuel" id="prix_mensuel" value="{{ old('prix_mensuel', $besoinHebergement->prix_mensuel) }}" step="1">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('localisation') ? 'is-invalid' : '' }}" name="localisation" id="localisation" required>{{ old('localisation', $besoinHebergement->localisation) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('geolocalisation') ? 'is-invalid' : '' }}" type="text" name="geolocalisation" id="geolocalisation" value="{{ old('geolocalisation', $besoinHebergement->geolocalisation) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('hostingtype') ? 'is-invalid' : '' }}" name="hostingtype_id" id="hostingtype_id" required>
                    @foreach($hostingtypes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('hostingtype_id') ? old('hostingtype_id') : $besoinHebergement->hostingtype->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('typeofhouse') ? 'is-invalid' : '' }}" name="typeofhouse_id" id="typeofhouse_id" required>
                    @foreach($typeofhouses as $id => $entry)
                        <option value="{{ $id }}" {{ (old('typeofhouse_id') ? old('typeofhouse_id') : $besoinHebergement->typeofhouse->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('setcountry') ? 'is-invalid' : '' }}" name="setcountry_id" id="setcountry_id" required>
                    @foreach($setcountries as $id => $entry)
                        <option value="{{ $id }}" {{ (old('setcountry_id') ? old('setcountry_id') : $besoinHebergement->setcountry->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('city') ? 'is-invalid' : '' }}" name="city_id" id="city_id" required>
                    @foreach($cities as $id => $entry)
                        <option value="{{ $id }}" {{ (old('city_id') ? old('city_id') : $besoinHebergement->city->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('city'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('quartier') ? 'is-invalid' : '' }}" name="quartier_id" id="quartier_id" required>
                    @foreach($quartiers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('quartier_id') ? old('quartier_id') : $besoinHebergement->quartier->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $besoinHebergement->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('liststatut') ? 'is-invalid' : '' }}" name="liststatut_id" id="liststatut_id" required>
                    @foreach($liststatuts as $id => $entry)
                        <option value="{{ $id }}" {{ (old('liststatut_id') ? old('liststatut_id') : $besoinHebergement->liststatut->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('libelle') ? 'is-invalid' : '' }}" type="text" name="libelle" id="libelle" value="{{ old('libelle', $besoinHebergement->libelle) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $besoinHebergement->description) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('emergencylevel') ? 'is-invalid' : '' }}" name="emergencylevel_id" id="emergencylevel_id" required>
                    @foreach($emergencylevels as $id => $entry)
                        <option value="{{ $id }}" {{ (old('emergencylevel_id') ? old('emergencylevel_id') : $besoinHebergement->emergencylevel->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('satisfait') ? 'is-invalid' : '' }}" type="text" name="satisfait" id="satisfait" value="{{ old('satisfait', $besoinHebergement->satisfait) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_satisfait') ? 'is-invalid' : '' }}" type="text" name="date_satisfait" id="date_satisfait" value="{{ old('date_satisfait', $besoinHebergement->date_satisfait) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('conveniences') ? 'is-invalid' : '' }}" type="text" name="conveniences" id="conveniences" value="{{ old('conveniences', $besoinHebergement->conveniences) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('servicesinclus') ? 'is-invalid' : '' }}" type="text" name="servicesinclus" id="servicesinclus" value="{{ old('servicesinclus', $besoinHebergement->servicesinclus) }}">
