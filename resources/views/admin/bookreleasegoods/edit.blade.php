...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.bookreleasegood.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.bookreleasegoods.update", [$bookreleasegood->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('releasegood') ? 'is-invalid' : '' }}" name="releasegood_id" id="releasegood_id" required>
                    @foreach($releasegoods as $id => $entry)
                        <option value="{{ $id }}" {{ (old('releasegood_id') ? old('releasegood_id') : $bookreleasegood->releasegood->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $bookreleasegood->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Bookreleasegood::CONFIRMATION_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('confirmation') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="confirmation_{{ $key }}" name="confirmation" value="{{ $key }}" {{ old('confirmation', $bookreleasegood->confirmation) === (string) $key ? 'checked' : '' }}>
