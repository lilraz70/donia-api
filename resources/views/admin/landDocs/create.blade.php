...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.landDoc.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.land-docs.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('typeadmdoc') ? 'is-invalid' : '' }}" name="typeadmdoc_id" id="typeadmdoc_id" required>
                    @foreach($typeadmdocs as $id => $entry)
                        <option value="{{ $id }}" {{ old('typeadmdoc_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
