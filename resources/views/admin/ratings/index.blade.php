...
@can('rating_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            ...
            @include('csvImport.modal', ['model' => 'Rating', 'route' => 'admin.ratings.parseCsvImport'])
        </div>
    </div>
@endcan
<div class="card">
    ...
    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-Rating">
            <thead>
                <tr>
                    <th width="10">
                ...
                <tr>
                    ...
                    <td>
                        <select class="search">
                            <option value>{{ trans('global.all') }}</option>
                            @foreach($rating_types as $key => $item)
                                <option value="{{ $item->intitule }}">{{ $item->intitule }}</option>
                            @endforeach
                        </select>
                    </td>
...
@endcan
  ...
  let dtOverrideGlobals = {
    ...
    retrieve: true,
    aaSorting: [],
    ajax: "{{ route('admin.ratings.index') }}",
    columns: [
      { data: 'placeholder', name: 'placeholder' },
{ data: 'id', name: 'id' },
{ data: 'areasofservices_intitule', name: 'areasofservices.intitule' },
{ data: 'objecttype_intitule', name: 'objecttype.intitule' },
{ data: 'user_name', name: 'user.name' },
{ data: 'ratingtype_intitule', name: 'ratingtype.intitule' },
{ data: 'actions', name: '{{ trans('global.actions') }}' }
  ...
  let table = $('.datatable-Rating').DataTable(dtOverrideGlobals);
