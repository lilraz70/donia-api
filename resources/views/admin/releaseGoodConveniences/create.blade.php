...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.releaseGoodConvenience.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.release-good-conveniences.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('conveniencetype') ? 'is-invalid' : '' }}" name="conveniencetype_id" id="conveniencetype_id" required>
                    @foreach($conveniencetypes as $id => $entry)
                        <option value="{{ $id }}" {{ old('conveniencetype_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
