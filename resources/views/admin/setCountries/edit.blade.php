...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.setCountry.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.set-countries.update", [$setCountry->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="intitule">{{ trans('cruds.setCountry.fields.intitule') }}</label>
                <input class="form-control {{ $errors->has('intitule') ? 'is-invalid' : '' }}" type="text" name="intitule" id="intitule" value="{{ old('intitule', $setCountry->intitule) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" type="text" name="code" id="code" value="{{ old('code', $setCountry->code) }}" required>
                @if($errors->has('code'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('prefix') ? 'is-invalid' : '' }}" type="text" name="prefix" id="prefix" value="{{ old('prefix', $setCountry->prefix) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('flag') ? 'is-invalid' : '' }}" type="text" name="flag" id="flag" value="{{ old('flag', $setCountry->flag) }}" required>
                @if($errors->has('flag'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('statut') ? 'is-invalid' : '' }}" name="statut" id="statut">
                    ...
                    @foreach(App\Models\SetCountry::STATUT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('statut', $setCountry->statut) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
