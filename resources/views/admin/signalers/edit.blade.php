...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.signaler.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.signalers.update", [$signaler->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="experience_utilisateur">{{ trans('cruds.signaler.fields.experience_utilisateur') }}</label>
                <input class="form-control {{ $errors->has('experience_utilisateur') ? 'is-invalid' : '' }}" type="text" name="experience_utilisateur" id="experience_utilisateur" value="{{ old('experience_utilisateur', $signaler->experience_utilisateur) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('comment') ? 'is-invalid' : '' }}" type="text" name="comment" id="comment" value="{{ old('comment', $signaler->comment) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('objet') ? 'is-invalid' : '' }}" type="number" name="objet" id="objet" value="{{ old('objet', $signaler->objet) }}" step="1" required>
                @if($errors->has('objet'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $signaler->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('objecttype') ? 'is-invalid' : '' }}" name="objecttype_id" id="objecttype_id" required>
                    @foreach($objecttypes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('objecttype_id') ? old('objecttype_id') : $signaler->objecttype->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('reason') ? 'is-invalid' : '' }}" name="reason_id" id="reason_id" required>
                    @foreach($reasons as $id => $entry)
                        <option value="{{ $id }}" {{ (old('reason_id') ? old('reason_id') : $signaler->reason->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
