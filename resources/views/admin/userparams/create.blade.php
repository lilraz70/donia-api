...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.userparam.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.userparams.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('parameterusertype') ? 'is-invalid' : '' }}" name="parameterusertype_id" id="parameterusertype_id" required>
                    @foreach($parameterusertypes as $id => $entry)
                        <option value="{{ $id }}" {{ old('parameterusertype_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('param_value') ? 'is-invalid' : '' }}" type="text" name="param_value" id="param_value" value="{{ old('param_value', '') }}" required>
