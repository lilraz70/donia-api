...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.userservice.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.userservices.update", [$userservice->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $userservice->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('areasofservice') ? 'is-invalid' : '' }}" name="areasofservice_id" id="areasofservice_id" required>
                    @foreach($areasofservices as $id => $entry)
                        <option value="{{ $id }}" {{ (old('areasofservice_id') ? old('areasofservice_id') : $userservice->areasofservice->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
