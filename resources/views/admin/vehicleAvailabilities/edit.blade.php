...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.vehicleAvailability.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.vehicle-availabilities.update", [$vehicleAvailability->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="jour_debut">{{ trans('cruds.vehicleAvailability.fields.jour_debut') }}</label>
                <input class="form-control date {{ $errors->has('jour_debut') ? 'is-invalid' : '' }}" type="text" name="jour_debut" id="jour_debut" value="{{ old('jour_debut', $vehicleAvailability->jour_debut) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('heure_debut') ? 'is-invalid' : '' }}" type="text" name="heure_debut" id="heure_debut" value="{{ old('heure_debut', $vehicleAvailability->heure_debut) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('jour_fin') ? 'is-invalid' : '' }}" type="text" name="jour_fin" id="jour_fin" value="{{ old('jour_fin', $vehicleAvailability->jour_fin) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('heure_fin') ? 'is-invalid' : '' }}" type="text" name="heure_fin" id="heure_fin" value="{{ old('heure_fin', $vehicleAvailability->heure_fin) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sellrentcar') ? 'is-invalid' : '' }}" name="sellrentcar_id" id="sellrentcar_id" required>
                    @foreach($sellrentcars as $id => $entry)
                        <option value="{{ $id }}" {{ (old('sellrentcar_id') ? old('sellrentcar_id') : $vehicleAvailability->sellrentcar->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
